...
```{r}
# set zero.policy = TRUE if we want to assign zero to the lagged value of zones without neighbors
#
...
ggplot(data = bperm.res, aes(x=bperm$res)) +
  ...
  geom_vline(xintercept = 0.000, color="red")
  
```

Note that our observed statistic, 0.300, falls to the right of the distribution suggesting that the GDPPC values are clustered (a positive Moran's I value suggests clustering whereas a negative Moran's I value suggests dispersion).
...
Since p-value \< 0.05, the level of significance, we reject the null hypothesis and conclude that the observed spatial pattern of GDPPC is **not** equally likely as any other spatial pattern (i.e. **not** spatially random)
