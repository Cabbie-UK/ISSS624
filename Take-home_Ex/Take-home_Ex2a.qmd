...
2.  We use `st_sf()` to convert the tibble data frame into sf data frame.
...
Shapefile downloaded from [geoBoundaries](https://www.geoboundaries.org/) portal will imported in st_read of sf package.
...
Although *wp_sf* sf data frame consists of a field called *#clean_adm2* which by right should provide the LGA name of the water point. However, it is always a good practice to be more caution when dealing with external data.
